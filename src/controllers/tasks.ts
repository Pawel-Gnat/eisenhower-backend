...
export const getAllTasks = async (req: express.Request, res: express.Response) => {
	...
	try {
		...
		responseHelpers.sendFulfilledResponseWithData(tasks, 200, 'Tasks loaded', Status.SUCCESS)
	} catch (error) {
		console.log(error)

		responseHelpers.sendRejectedResponse(500, 'Internal server error', Status.DANGER)
	}
}
...
export const createNewTask = async (req: express.Request, res: express.Response) => {
	...
	try {
		...
		if (!title) {
			return responseHelpers.sendRejectedResponse(400, 'Title is required', Status.WARNING)
		}

		const existingTask = await getTaskByTitle(userId, title)

		if (existingTask) {
			return responseHelpers.sendRejectedResponse(400, 'Title already exists', Status.WARNING)
		}
...
export const updateTask = async (req: express.Request, res: express.Response) => {
	...
	try {
		...
		responseHelpers.sendFulfilledResponseWithData(task, 200, 'Task updated', Status.SUCCESS)
	} catch (error) {
		console.log(error)
