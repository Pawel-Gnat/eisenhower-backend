...
export const loginUser = async (req: express.Request, res: express.Response) => {
	...
	try {
		...
		if (!email || !password) {
			return responseHelpers.sendRejectedResponse(400, 'All fields are required', Status.WARNING)
		}

		const user = await getUserByEmail(email).select('+password')

		if (!user) {
			return responseHelpers.sendRejectedResponse(400, 'Invalid email or password', Status.WARNING)
		}
		...
		if (!session) {
			...
			try {
				...
				if (error instanceof jwt.TokenExpiredError) {
					...
					return responseHelpers.sendRejectedResponse(400, 'Invalid session token', Status.DANGER)
				}
			}
		}
...
export const logoutUser = async (req: express.Request, res: express.Response) => {
	...
	const responseHelpers = createResponseHelpers(res)
	responseHelpers.sendFulfilledResponseWithoutData(204, 'Logged out', Status.SUCCESS)
}
...
export const registerUser = async (req: express.Request, res: express.Response) => {
	...
	try {
		...
		if (!emailRegex.test(email)) {
			return responseHelpers.sendRejectedResponse(400, 'Invalid email format', Status.WARNING)
		}
		...
		if (existingUser) {
			return responseHelpers.sendRejectedResponse(400, 'User already exists', Status.WARNING)
		}
		...
		responseHelpers.sendFulfilledResponseWithoutData(200, 'User registered successfully', Status.SUCCESS)
	} catch (error) {
		console.log(error)
		return responseHelpers.sendRejectedResponse(500, 'Internal server error', Status.DANGER)
	}
